...
"""Price_x and Price_y columns have the same value in each column so we can drop one of them"""
...
# drop missing values ​​because the number is not significant
...
"""P-Value less than 0.05 indicates that the data is stationary and can be utilized in ARIMA time series analysis."""
...
"""The Autocorrelation plot (ACF) indicates an order of 2 (p) as the first and second lag fall well outside the significance limit. Conversely, the Partial Autocorrelation plot (PACF) suggests an order of 3 (q) because of the significant correlation observed up to the third lag.
...
# Normalize dataset using preprocessing
...
"""###### The best cluster (k) is found in 4 clusters"""
